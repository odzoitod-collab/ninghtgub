...
export const FilterSheet: React.FC<Props> = ({ isOpen, onClose, onApply, currentFilters }) => {
  ...
  const NumberInput = ({ label, value, onChange, placeholder }: { label?: string, value: number, onChange: (val: number) => void, placeholder: string }) => (
    <div className="flex-1 bg-dark-800 rounded-2xl border border-dark-700 focus-within:border-violet-500 focus-within:ring-1 focus-within:ring-violet-500/50 transition-all relative">
      <input 
        type="number" 
        value={value || ''}
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isCityExpanded ? 'max-h-[60vh] border-t border-dark-700' : 'max-h-0'}`}>
              <div className="overflow-y-auto max-h-[60vh] bg-dark-800">
                ...
                >
                  ...
                  {localFilters.city === "" && <CheckIcon className="w-5 h-5 text-violet-500" />}
                </div>
                {CITIES.map(city => (
                  <div 
                    key={city}
                  ...
                  >
                    ...
                    {localFilters.city === city && <CheckIcon className="w-5 h-5 text-violet-500" />}
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="flex-none p-5 bg-dark-900 border-t border-dark-800 pb-8 safe-bottom">
        <div className="flex space-x-3">
          ...
          <button 
            ...
            className="flex-1 bg-gradient-to-r from-violet-400 to-violet-600 text-white font-bold py-4 rounded-xl text-lg shadow-lg shadow-violet-500/20 active:scale-[0.98] transition-transform"
          >
            Применить
          </button>
        </div>
