  ...
  const NavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => {
    ...
    return (
      <button 
        ...
        className={`flex flex-col items-center justify-center space-y-1 w-full h-full transition-colors duration-200 ${isActive ? 'text-violet-500' : 'text-gray-500 hover:text-gray-300'}`}
      >
